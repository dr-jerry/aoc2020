...
object Eleven extends App {
  ...
  var array = Source.fromInputStream(this.getClass.getResourceAsStream("eleven.txt")).getLines()
  ...
  def evaluate(field: String): String = {
    ...
    array = field.split("\n").map(s => s.toCharArray)
    val newField = (0 to boundary.y).map(y => {(0 to boundary.x).map(x =>
