...
object Nine extends App {
  val pLength = 25
  val lines = Source.fromInputStream(this.getClass.getResourceAsStream("nine.txt")).getLines()
  ...
  (3 to 30).foreach(m => {
    ...
    (0 to arry.length-m).foreach(s => {
      ...
      if (slice.sum == wrong) {slice.foreach(l => println(s">> $l"));println(s"->${slice(0) + slice(slice.length-1)}")}
  })})
