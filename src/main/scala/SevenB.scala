...
//    ruleMap.getOrElse()
//  }
  val lines = Source.fromInputStream(this.getClass.getResourceAsStream("seven.txt")).getLines().toList

  val ruleMap = mutable.Map(lines.map(l => { val a = l.split(" contain ")
    val set = a(1).split(',').map(str => {val t = sanitize(str)
      s"(${t._1} * '${t._2}')"}).toSet
      sanitize(a(0))._2 -> set}): _*)
  ...
  def calculate(st: Settimes) :BigInt = {
    st match {
      case Settimes(1, "no other", _) => {println(1); 1 }
