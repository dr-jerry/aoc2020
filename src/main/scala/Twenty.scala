...
object Twenty extends App {
  ...
  val lines = Source.fromInputStream(this.getClass.getResourceAsStream("twenty-test.txt")).getLines().toList
  ...
  pieces.foreach(pc => {addToMap(pc.top, pc);addToMap(pc.left, pc)
                            addToMap(pc.bottom, pc);addToMap(pc.right, pc)})
...
//    .foreach(s => println(s.mkString(",")))
  ...
  val corners = pieces.map(pc => (pc, sides2Pieces.values.filter(s => {/*println(s"${pc.id} -> $s -> ${s.contains(pc)}");*/
    s.size > 1 && s.contains(pc)}).size)).filter(t => t._2 <= 4)
